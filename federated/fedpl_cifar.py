...
import copy
from nets.models import PLCIFARModel
import argparse
import numpy as np
...
if __name__ == '__main__':
    ...
    server_model = PLCIFARModel(class_num=args.classnum).to(device)
    ...
    for a_iter in range(args.iters):
        ...
        if not os.path.exists(os.path.join('../logs/cifar_fedpl', args.mode)):
            os.makedirs(os.path.join('../logs/cifar_fedpl', args.mode))
    ...
    # save record
    np.savetxt(os.path.join('../logs/cifar_fedpl', args.mode, 'client' + str(args.clientnum) +
