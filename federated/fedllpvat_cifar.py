...
import copy
from nets.models import LLPCIFARModel
import argparse
import numpy as np
...
def train(args, model, train_loader, optimizer, loss_fun, client_num, device, Pi, priors_corr, prior_test, bagsize):
    ...
    consis_loss_func = VATLoss(xi=1e-6, eps=6.0, ip=1)
    for step in range(len(train_iter)):
        ...
        alpha = get_rampup_weight(0.05, step, consistency_rampup)
...
if __name__ == '__main__':
    ...
    if not os.path.exists(os.path.join('../logs/cifar_fedllpvat', args.mode)):
        os.makedirs(os.path.join('../logs/cifar_fedllpvat', args.mode))

    exp_folder = 'cifar_fedllpvat'
    training_loss_log = []
    error_rate_log = []
    ...
    server_model = LLPCIFARModel(class_num=args.classnum).to(device)
    loss_fun = nn.MSELoss()
    ...
    # save record
    np.savetxt(os.path.join('../logs/cifar_fedllpvat', args.mode, 'client' + str(args.clientnum) +
