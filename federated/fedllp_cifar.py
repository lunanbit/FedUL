...
def train(args, model, train_loader, optimizer, loss_fun, client_num, device, Pi, priors_corr, prior_test, bagsize):
    ...
    for step in range(len(train_iter)):
        ...
        loss = prop_loss + L1_Regularization(model) * args.wdecay
...
if __name__ == '__main__':
    ...
    if not os.path.exists(os.path.join('../logs/cifar_fedllp', args.mode)):
        os.makedirs(os.path.join('../logs/cifar_fedllp', args.mode))

    exp_folder = 'cifar_fedllp'
    training_loss_log = []
    error_rate_log = []
    ...
    # save record
    np.savetxt(os.path.join('../logs/cifar_fedllp', args.mode, 'client' + str(args.clientnum) +
